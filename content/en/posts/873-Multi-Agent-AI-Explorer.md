...
A `Triage` agent understands a customer's query and routes it to a specialized `Billing_Agent` or `Technical_Support_Agent` for resolution.
...
A `Chat_Agent` collects user preferences, an `Information_Agent` queries APIs for flights and hotels, and a `Route_Agent` plans the optimal itinerary.
...
A `Job_Analyzer` agent reads a job description, and a `Resume_Writer` agent rewrites a base resume to highlight the most relevant skills and experience.
